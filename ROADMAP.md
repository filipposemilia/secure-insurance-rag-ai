...
| 8 | **Qualità delle risposte e trasparenza**: groundedness misurata sulle cifre invece che sulle parole (ADR-022), streaming con l'output guard **interposto** invece che successivo (ADR-023), citazioni con l'estratto verificabile, catalogo dei dieci rischi OWASP nella scheda Sicurezza con profilo modificabile (ADR-024) | ✅ |
...
| :--- | :--- | :--- | :--- |
| **Misura dei falsi negativi del livello 2** | Oggi si sa che il NER *aggiunge* copertura — nomi in testo libero che le regex non vedono — ma **non quanto ne manchi**: un rilevatore probabilistico che non trova un nome non produce alcun segnale, quindi la lacuna è invisibile per costruzione. Senza una misura, «copriamo i nomi in testo libero» è un'affermazione senza numero dietro | Serve un corpus annotato a mano: qualche decina di documenti sintetici con le entità marcate, su cui calcolare quante ne sfuggono. Il lavoro è nell'annotazione, non nel codice | ~1 g |
...
| **Irrobustimento della catena di fornitura** (LLM05) | È la voce più scoperta del mapping OWASP, ed è emersa contandolo: firma delle immagini, SBOM generato in build, scansione delle vulnerabilità delle dipendenze in integrazione continua. Oggi c'è solo il fissaggio delle versioni — necessario, non sufficiente | Richiede una pipeline di CI che non esiste: il progetto si costruisce a mano sul VPS. Ha senso insieme a un flusso di build automatico, non prima | ~4 h + CI |
| **Groundedness valutata da un modello** (ADR-022) | Il controllo attuale verifica che ogni cifra della risposta esista nel contesto: intercetta il massimale inventato, ma non una cifra corretta attribuita alla voce sbagliata («franchigia 5.000.000»), né un'affermazione falsa senza numeri | Un valutatore basato su modello raddoppia le chiamate e va tarato su un dataset di riferimento che non esiste. Si sovrappone alla voce sulla valutazione automatica più sotto | ~1 g |
